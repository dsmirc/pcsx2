...
void GSDeviceVK::RenderHW(GSHWDrawConfig& config)
{
	...
	{
		if (draw_rt && draw_rt->GetLayout() != GSTextureVK::Layout::ReadWriteImage)
		{
			EndRenderPass();
			draw_rt->CommitClear();
		...
		else
		{
			...
			if (m_current_pipeline_layout != PipelineLayout::ROVTFX)
		...
		const VkViewport vp{0.0f, 0.0f, static_cast<float>(config.rt->GetWidth()),
			static_cast<float>(config.rt->GetHeight()), 0.0f, 1.0f};
		SetViewport(vp);
