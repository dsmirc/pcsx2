...
GSTextureCache::Source* GSTextureCache::CreateMergedSource(GIFRegTEX0 TEX0, GIFRegTEXA TEXA, SourceRegion region, const GSVector2& scale)
{
	...
	const int tex_width = 64 * TEX0.TBW;
	...
	GSTexture* copy_src = nullptr;
	GSDevice::MultiStretchRect* copy_queue =
		static_cast<GSDevice::MultiStretchRect*>(alloca(sizeof(GSDevice::MultiStretchRect) * num_pages));
	u32 copy_count = 0;
	...
	for (int page_num = 0; page_num < num_pages; page_num++)
	{
		...
		bool found = false;
		GL_INS("Searching for block range %x - %x for (%u,%u)", this_start_block, this_end_block, page_x* page_width, page_y* page_height);
		...
		for (auto i = m_dst[RenderTarget].begin(); i != m_dst[RenderTarget].end(); ++i)
		{
			...
			if (this_start_block >= t->m_TEX0.TBP0 && this_end_block <= t->m_end_block && t->m_TEX0.PSM == TEX0.PSM)
			{
				...
				for (int copy_page_y = 0; copy_page_y < available_pages_y; copy_page_y++)
				{
					...
					const int copy_height = std::min(std::min(src_y_end - src_y, tex_height - dst_y), page_height);
					...
					for (int copy_page_x = 0; copy_page_x < available_pages_x; copy_page_x++)
					{
						...
						if ((pages_done[row_page / 8] & (1u << (row_page % 8))) == 0)
						{
							...
							const int copy_width = std::min(std::min(src_x_end - src_x, tex_width - dst_x), page_width);
		...
		if (!found)
		{
			if (GSConfig.PreloadFrameWithGSData)
			{
				...
				const GSVector4i rect(dst_x, dst_y, dst_x + page_width, dst_y + page_height);

				if (lmtex_mapped)
				{
					psm.rtx(g_gs_renderer->m_mem, lm_off, rect, lmtex_map.bits + dst_y * lmtex_map.pitch + dst_x * sizeof(u32), lmtex_map.pitch, TEXA);
				}
				else
				{
