...
GSTextureCache::Target* GSTextureCache::LookupTarget(GIFRegTEX0 TEX0, const GSVector2i& size, float scale, int type,
	bool used, u32 fbmask, bool is_frame, bool is_clear, bool preload, bool preserve_target, const GSVector4i draw_rect)
{
	const GSLocalMemory::psm_t& psm_s = GSLocalMemory::m_psm[TEX0.PSM];
	const u32 bp = TEX0.TBP0;
	...
	if (dst)
	{
		...
		if (!dst->m_dirty.empty() && !preserve_target && draw_rect.rintersect(dst->m_valid).eq(dst->m_valid))
		{
	...
	if (!dst)
	{
		...
		if (TEX0.TBW > 0 && supported_fmt)
		{
			...
			if (!is_frame && !preload)
			{
				...
				{
					const u32 rect_end = GSLocalMemory::GetEndBlockAddress(TEX0.TBP0, TEX0.TBW, TEX0.PSM, newrect);
