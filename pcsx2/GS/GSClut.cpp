...
bool GSClut::WriteState::IsDirty(const GIFRegTEX0& TEX0_, const GIFRegTEXCLUT& TEXCLUT_)
{
	constexpr u64 mask = 0x1FFFFFE000000000ull; // CSA CSM CPSM CBP

	bool is_dirty = dirty;

	if (((this->TEX0.U64 ^ TEX0_.U64) & mask) || (GSLocalMemory::m_psm[this->TEX0.PSM].pal != GSLocalMemory::m_psm[TEX0_.PSM].pal))
		is_dirty |= true;
	else if (TEX0_.CSM == 1 && (TEXCLUT_.U32[0] ^ this->TEXCLUT.U32[0]))
		is_dirty |= true;

	if (!is_dirty)
	{
		TEX0.U64 = TEX0_.U64;
		TEXCLUT.U64 = TEXCLUT_.U64;
	}

	return is_dirty;
}

bool GSClut::ReadState::IsDirty(const GIFRegTEX0& TEX0_)
{
	constexpr u64 mask = 0x1FFFFFE000000000ull; // CSA CSM CPSM CBP

	bool is_dirty = dirty;

	if (((TEX0.U64 ^ TEX0_.U64) & mask) || (GSLocalMemory::m_psm[this->TEX0.PSM].pal != GSLocalMemory::m_psm[TEX0_.PSM].pal))
		is_dirty |= true;
...
bool GSClut::ReadState::IsDirty(const GIFRegTEX0& TEX0_, const GIFRegTEXA& TEXA_)
{
	...
	if (((TEX0.U64 ^ TEX0_.U64) & tex0_mask) || (GSLocalMemory::m_psm[this->TEX0.PSM].pal != GSLocalMemory::m_psm[TEX0_.PSM].pal))
		is_dirty |= true;
	else // Just to optimise the checks.
	{
		// Check TA0 and AEM in 24bit mode.
		if (TEX0_.CPSM == PSMCT24 && ((TEXA.U64 ^ TEXA_.U64) & texa24_mask))
			is_dirty |= true;
		// Check all fields in 16bit mode.
		else if (TEX0_.CPSM >= PSMCT16 && ((TEXA.U64 ^ TEXA_.U64) & texa16_mask))
			is_dirty |= true;
	}
