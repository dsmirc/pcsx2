...
public:
	...
	bool FreezeIn(zip_file_t* zf) const override { return SysState_ComponentFreezeIn(zf, SPU2); }
	bool FreezeOut(SaveStateBase& writer) const override { return SysState_ComponentFreezeOut(writer, SPU2); }
...
std::unique_ptr<ArchiveEntryList> SaveState_DownloadState(Error* error)
{
	...
	{
		Error::SetUser(error, "FreezeBios() failed", "The log may contain more information.");
		return false;
	}

	if (!saveme.FreezeInternals())
	{
		Error::SetUser(error, "FreezeInternals() failed", "The log may contain more information.");
		return false;
	}
	...
	for (const std::unique_ptr<BaseSavestateEntry>& entry : SavestateEntries)
	{
		...
		{
			Error::SetUser(error, fmt::format("FreezeOut() failed for {}.", entry->GetFilename()), "The log may contain more information.");
			destlist.reset();
			break;
		}
...
static bool CheckVersion(const std::string& filename, zip_t* zf, Error* error)
{
	...
	{
		Error::SetUser(error, "Savestate file does not contain version indicator.", "This file is not a valid PCSX2 savestate.  See the logfile for details.");
		return false;
	}
	...
	{
		Error::SetUser(error, fmt::format("Savestate uses an unsupported or unknown savestate version.\n(PCSX2 ver={:x}, state ver={:x})", g_SaveVersion, savever),
	...
	{
		Error::SetUser(error, fmt::format("Savestate uses an unknown savestate version.\n(PCSX2 ver={:x}, state ver={:x})", g_SaveVersion, savever),
			"Cannot load this savestate. The state is an unsupported version.\nOption 1: Download an older PCSX2 version from pcsx2.net and make a memcard save like on the physical PS2.\nOption 2: Delete the savestates.");
		return false;
	}
...
bool SaveState_UnzipFromDisk(const std::string& filename, Error* error)
{
	...
	if (!zf)
	{
		...
		Error::SetUser(error, "Savestate file is not a valid gzip archive.",
			"This savestate cannot be loaded because it is not a valid gzip archive.  It may have been created by an older unsupported version of PCSX2, or it may be corrupted.");
		return false;
	}
	...
	if (!allPresent)
	{
		Error::SetUser(error, "Savestate cannot be loaded: some required components were not found or are incomplete."
								"This savestate cannot be loaded due to missing critical components.  See the log file for details.");
		return false;
	}
	...
	{
		Error::SetUser(error, "Save state corruption in internal structures.",
			"This save state cannot be loaded.");
		return false;
	}
	...
	for (u32 i = 0; i < std::size(SavestateEntries); ++i)
	{
		...
		{
			Error::SetUser(error, fmt::format("Save state corruption in {}.", SavestateEntries[i]->GetFilename()),
				"This save state cannot be loaded.");
			return false;
		}
	}
