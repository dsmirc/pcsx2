...
struct FPControlRegister
{
	...
	static constexpr u64 RMODE_MASK = (0x3ULL << RMODE_SHIFT);
...
#else
		return FPControlRegister{_ReadStatusReg(0x5A20)};
#endif
	}
...
#else
		_WriteStatusReg(0x5A20, value.bitmask);
#endif
	}
	...
	__fi constexpr FPRoundMode GetRoundMode() const
	{
		...
		const u64 RMode = (bitmask >> ROUNDING_CONTROL_SHIFT) & ROUNDING_CONTROL_MASK;
	...
	__fi constexpr FPControlRegister& SetRoundMode(FPRoundMode mode)
	{
		...
		bitmask = (bitmask & ~ROUNDING_CONTROL_BITS) | ((RMode & ROUNDING_CONTROL_MASK) << ROUNDING_CONTROL_SHIFT);
		return *this;
	}
